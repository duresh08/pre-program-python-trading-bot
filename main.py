...
def finding_3_mo_returns(df):
    ...
    df_final = df_final.drop(columns=['target_date_initial', 'adj_close_3_mo'])
...
def lightgbm_categorical_training_pipeline(X_train, y_train, train_weights, X_cv=None, y_cv=None, cv_weights=None,
                                           space=None, early_stopping_rounds=50, verbose=1000,
...
def pull_data_from_db(start_date, end_date, db_path):
    ...
    tickdates = get_all_db_tickdates(db_path)

    # finding and snapping date range to the closest trading date available in the db
    start_date = str(tickdates[(tickdates['date'].astype(str) <= start_date)]['date'].max())
    end_date = str(tickdates[(tickdates['date'].astype(str) >= end_date)]['date'].min())

    db_path = "sqlite:///{}".format(db_path)
    df = pd.read_sql("SELECT * FROM signals WHERE date >= '{}' AND date <= '{}'".format(
        start_date, end_date), db_path)
    return df


@lru_cache(maxsize=1)
def get_all_db_tickdates(db_path):
    print("initializing db tickdates")
    db_path = "sqlite:///{}".format(db_path)
    tickdates = pd.read_sql("SELECT DISTINCT date FROM signals ORDER BY date;", db_path)
    tickdates = tickdates.reset_index(drop=True)
    return tickdates
...
def walk_forward_simulation(hyperparameters, train_n_months=15, categorical_features=None,
                            prediction_date="2006-04-01",
    ...
    tickdates = get_all_db_tickdates(db_path)
    while prediction_date < final_end_date:
        # take 18 months of data prior to the timestep we are predicting
        pull_db_start_date = (pd.to_datetime(prediction_date, format='%Y-%m-%d') -
                              pd.DateOffset(months=train_n_months + delta_months)).strftime("%Y-%m-%d")
        pull_db_end_date = (dt.datetime.strptime(prediction_date, "%Y-%m-%d") +
                            dt.timedelta(days=10)).strftime("%Y-%m-%d")

        # pulling and processing data
        df = pull_data_from_db(pull_db_start_date, pull_db_end_date, db_path)
        df = pre_process_data(df)
        df = finding_3_mo_returns(df)

        # splitting train and test packet data to avoid leakage
        # purge last 3 months of data from prediction date from the full data we pulled from db to avoid leakage
        train_filter = (pd.to_datetime(prediction_date, format='%Y-%m-%d') -
                        pd.DateOffset(months=delta_months)).strftime("%Y-%m-%d")
        train_df = df[(df['target_date'].astype(str) < train_filter)].drop(columns=['target_date']).reset_index(
            drop=True)

        # snapping test packet tickdate to the nearest on available in db for prediction
        test_tickdate = tickdates[(tickdates['date'].astype(str) >= train_filter)]['date'].min()
        test_df = df[(df['date'] == test_tickdate)].reset_index(drop=True)
        prediction_for_tickdate = str(test_df['target_date'].min().strftime("%Y-%m-%d"))
        test_df = test_df.drop(columns=['target_date'])

        # log
        print("train_df min_date: {}, max_date: {}".format(train_df['date'].min(), train_df['date'].max()))
        print("test_df min_date: {}, max_date: {}".format(test_df['date'].min(), test_df['date'].max()))

        # training lightgbm model
        model = lightgbm_model_training(train_df, hyperparameters, cv_df=None,
                                        categorical_features=categorical_features)

        # making predictions
        X_test, categorical_indices, y_test = categorify_and_return(categorical_features, test_df)
        y_pred = model.predict(X_test).reshape(X_test.shape[0], 1)
        test_df['y_pred'] = y_pred
        test_df = test_df.sort_values(['y_pred'], ascending=False).head(top_n_picks).reset_index(drop=True)

        # calculating pnl after ranking picls
        pnl = (((1 + test_df['returns_3_mo']) * stock_allocation).sum() -
               top_n_picks * stock_allocation)
        master_pnl_df = pd.concat([master_pnl_df, pd.DataFrame({'prediction_made_on': [test_tickdate],
                                                                'prediction_made_for_date': [prediction_for_tickdate],
                                                                'pnl': [pnl]})]).reset_index(drop=True)

        # incrementing timestep for the next prediction
...
if __name__ == '__main__':
    ...
    db_path = "C:\\Users\\USER\\Downloads\\data.db\\data.db"
    ...
    walk_forward_simulation(lgb_hyperparameters, train_n_months=15, categorical_features=categorical_features,
                            prediction_date="2006-04-01",
                            final_end_date="2024-12-31", delta_months=3,
                            db_path=db_path)
